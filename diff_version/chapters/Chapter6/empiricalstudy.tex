...
In this study, we \DIFdelbegin \DIFdel{qualitatively analyze }\DIFdelend \DIFaddbegin \DIFadd{perform both qualitative and quantitative analyses with
respect to }\DIFaddend the delivery delay phenomena\DIFaddbegin \DIFadd{. We gather our data }\DIFaddend by surveying and
interviewing the team members \DIFaddbegin \DIFadd{(}\ie \DIFadd{participants) }\DIFaddend of our subject projects. In
this section, we describe the subject projects \DIFaddbegin \DIFadd{and }\DIFaddend how we collect and analyze
our data.

\subsection{Subjects}

We \DIFdelbegin \DIFdel{analyze }\DIFdelend \DIFaddbegin \DIFadd{surveyed 37 participants of }\DIFaddend the Firefox, ArgoUML, and Eclipse (JDT) projects.
We naturally choose these projects, since \DIFdelbegin \DIFdel{this qualitative }\DIFdelend \DIFaddbegin \DIFadd{the analyses of this }\DIFaddend study is intended
...
able to study the perceived impact of \DIFdelbegin \DIFdel{the shift between release strategies }\DIFdelend \DIFaddbegin \DIFadd{adopting rapid releases }\DIFaddend when surveying the
participants of the Firefox project, we \DIFdelbegin \DIFdel{study the opinion of the ArgoUML and Eclipse participants about how that }\DIFdelend \DIFaddbegin \DIFadd{ask the participants of the other
projects to conjecture how such an }\DIFaddend impact would be \DIFdelbegin \DIFdel{on their
projects}\DIFdelend \DIFaddbegin \DIFadd{(}\ie \DIFadd{the impact on delivery
delays)}\DIFaddend . 
...
To collect the data to perform our \DIFdelbegin \DIFdel{qualitative }\DIFdelend \DIFaddbegin \DIFadd{third }\DIFaddend study, we design a web-based survey
that was sent to 780 participants of the Firefox, Eclipse (JDT), and ArgoUML
projects. \DIFaddbegin \DIFadd{Before sending our survey to the participants of the studied projects,
...
\DIFaddend We sent our \DIFaddbegin \DIFadd{polished }\DIFaddend survey to 513 Firefox, 184 Eclipse (JDT), and 83 ArgoUML
participants. We \DIFdelbegin \DIFdel{gather developer }\DIFdelend \DIFaddbegin \DIFadd{gathered the participants' }\DIFaddend e-mails from the respective
developer mailing list archives of \DIFdelbegin \DIFdel{the subject projects}\DIFdelend \DIFaddbegin \DIFadd{each studied project}\DIFaddend . We consider e-mail
addresses from messages that were sent in the past 4 years. To encourage
participation, we provided \DIFaddbegin \DIFadd{\$100 Amazon.com }\DIFaddend gift cards to a random subset of the
...
\DIFaddbegin \DIFadd{particular, we are interested on understanding the perceived advantages and disadvantages of
adopting rapid releases with respect to delivery delays. In
}\DIFaddend \hyperref[tbl:survey]{Table}~\ref{tbl:survey}, we highlight a subset of the
...
participants. \DIFdelbegin \DIFdel{We also conduct }\DIFdelend \DIFaddbegin \DIFadd{During our survey, we ask if the participant is willing to perform
a }\DIFaddend follow-up \DIFdelbegin \DIFdel{interviews with four of the Firefox
participants to }\DIFdelend \DIFaddbegin \DIFadd{interview. In total, we perform 4 follow-up interviews (from
the 8 participants that declared to be willing to do so).  
}

\DIFadd{The goal of our follow-up interviews is to }\DIFaddend gather deeper insights into \DIFdelbegin \DIFdel{their responses . }\DIFdelend \DIFaddbegin \DIFadd{the
responses of the participants. In particular, we intend to }{\em \DIFadd{(i)}} \DIFadd{better
understand the reasons as to why they assign a given rank to the factors that
are presented in }{\em \DIFadd{question \#13}}\DIFadd{, }{\em \DIFadd{(ii)}} \DIFadd{gather more details about why
do they agree or disagree with our quantitative data of prior studies, and }{\em
\DIFadd{(iii)}} \DIFadd{recover feedback when they do not answer a specific question or do not
understand it.  }\DIFaddend Our interviews are semi-structured \DIFdelbegin \DIFdel{and our goal was to clarify the responses }\DIFdelend \DIFaddbegin \DIFadd{(}\ie \DIFadd{we did not strictly
follow our script in case our interviewee provides more subjects to discuss).
For the interested reader, the invitation letter }\DIFaddend of our survey and \DIFdelbegin \DIFdel{collect more details about specific cases of delivery delays for addressed
issues}\DIFdelend \DIFaddbegin \DIFadd{the script
that guided our interviews are available at
}\hyperref[invitation_letter]{Appendices}\DIFadd{~\ref{invitation_letter}
and~\ref{interview_script}}\DIFaddend .

\subsection{Research Approach}

Given the \DIFdelbegin \DIFdel{exploratory nature of our qualitative analysis, we use methods from
}%DIFDELCMD < {\em %%%
\DIFdel{Grounded Theory}%DIFDELCMD < }%%%
\DIFdel{~\mbox{%DIFAUXCMD
\cite{groundtheorybook}}%DIFAUXCMD
. The }\DIFdelend \DIFaddbegin \DIFadd{geographic limitations to perform in-person meetings, the }\DIFaddend author of
...
unique codes. Finally, we \DIFdelbegin \DIFdel{used axial coding to find higher level conceptual themes }\DIFdelend \DIFaddbegin \DIFadd{group our codes into higher-level conceptual }{\em
\DIFadd{themes}} \DIFadd{in order }\DIFaddend to answer our RQs. \DIFaddbegin \DIFadd{The codes that were produced during our open
...
based on the received responses rather than scored by \DIFaddbegin \DIFadd{the }\DIFaddend participants. Also, we
mention quotes from the interviews when necessary to provide more detail about
the results\DIFdelbegin \DIFdel{and to allow for traceability through links to raw data}\DIFdelend . Finally, \hyperref[tbl:participants]{Table}~\ref{tbl:participants}
...
specific comparisons. For example, the Dunn test indicates \DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{whether }\DIFaddend the ranks
...
do \DIFdelbegin \DIFdel{that}\DIFdelend \DIFaddbegin \DIFadd{so}\DIFaddend , we use Spearman rank {$\rho$} correlation~\cite{spearman1904proof}, which
...
\subsection{\DIFdelbegin \DIFdel{Exploratory Analysis}\DIFdelend \DIFaddbegin \DIFadd{Demographics}\DIFaddend }\label{subsubsec:exploratory}
...
\end{figure}
We present \DIFdelbegin \DIFdel{a exploratory analysis of the data that we collect from the responses
of the participants}\DIFdelend \DIFaddbegin \DIFadd{the demographics of our obtained data}\DIFaddend .
...
\subsection{RQ1: What are developers' perceptions as to why \DIFdelbegin \DIFdel{integration }\DIFdelend \DIFaddbegin \DIFadd{delivery }\DIFaddend delays
occur?}\label{ch5:rq1}
...
\begin{sloppypar} Our findings about developers' perceptions of the causes of
	...
	\DIFaddbegin \DIFadd{and }\DIFaddend {\em (iv) \DIFdelbegin \DIFdel{frustration}\DIFdelend \DIFaddbegin \DIFadd{team collaboration}\DIFaddend }\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{. Also, we observe that }\DIFaddend {\em (v)
	\DIFdelbegin \DIFdel{team
collaboration}\DIFdelend \DIFaddbegin \DIFadd{frustration}\DIFaddend } \DIFaddbegin \DIFadd{is a main perceived consequence of delivery delays}\DIFaddend . After
...
\noindent\DIFdelbegin \textit{\textbf{\DIFdel{Theme~1---Development activities.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \theme{th:1} 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \finding{Development activities.}{find25} 
...
\DIFdelbegin \DIFdel{P17}\DIFdelend \DIFaddbegin \DIFadd{F17}\DIFaddend } states that the lack of {\em ``actual user testing beyond what QA can
...
\DIFdelbegin \DIFdel{P19}\DIFdelend \DIFaddbegin \DIFadd{F19}\DIFaddend }, delivery delay may happen because {\em ``testing has been too
...
\noindent\DIFdelbegin \textit{\textbf{\DIFdel{Theme~2---Decision making.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \theme{th:2} %%%
\DIFdelend \DIFaddbegin \finding{Decision making.}{find26} \DIFaddend Decision making refers to the
...
\noindent\DIFdelbegin \textit{\textbf{\DIFdel{Theme~3---Risk.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \theme{th:3} %%%
\DIFdelend \DIFaddbegin \finding{Risk.}{find27} \DIFaddend The risk that is associated with shipping
...
\noindent\DIFdelbegin \textit{\textbf{\DIFdel{Theme~4---Frustration.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \theme{th:4} %%%
\DIFdelend \DIFaddbegin \finding{Collaboration with other teams.}{find28}
\DIFaddend Delivery delay may \DIFaddbegin \DIFadd{also occur due to
the overhead that is introduced when }{\em \DIFadd{collaboration}}\DIFadd{$^{(10)}$ is needed
between teams. For example, when asked to recall a delayed addressed
issue, }{\em \DIFadd{F23}} \DIFadd{answers that }{\em \DIFadd{``sometimes, issues that require cross-team
cooperation may be delayed when the issue is differently prioritized by each
team.''}} \DIFadd{The }{\em \DIFadd{marketing}}\DIFadd{$^{(5)}$ team is mentioned recurrently when
delivery delay occurs due to other teams' collaboration. For instance,
according to }{\em \DIFadd{F21}}\DIFadd{, delivery delay }{\em \DIFadd{``generally happens when
marketing wants to make a splash.''}} {\em \DIFadd{F08}} \DIFadd{also corroborates  }{\em \DIFadd{F21}} \DIFadd{by
stating that }{\em \DIFadd{``product management }[\DIFadd{may}] \DIFadd{change their mind about the
...
\noindent\finding{Frustration.}{find29} \DIFadd{Delivery delay may }\DIFaddend generate
...
\noindent\DIFdelbegin \textit{\textbf{\DIFdel{Theme~5---Collaboration with other teams.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \theme{th:5}
%DIFDELCMD < %%%
\DIFdel{Delivery delay may also occur due to
the overhead that is introduced when }%DIFDELCMD < {\em %%%
\DIFdel{collaboration}%DIFDELCMD < }%%%
\DIFdel{$^{(10)}$ is needed
between teams. For example, when asked to recall a delayed addressed
issue, }%DIFDELCMD < {\em %%%
\DIFdel{F23}%DIFDELCMD < } %%%
\DIFdel{answers that }%DIFDELCMD < {\em %%%
\DIFdel{``sometimes, issues that require cross-team
cooperation may be delayed when the issue is differently prioritized by each
team.''}%DIFDELCMD < } %%%
\DIFdel{The }%DIFDELCMD < {\em %%%
\DIFdel{marketing}%DIFDELCMD < }%%%
\DIFdel{$^{(5)}$ team is mentioned recurrently when
delivery delay occurs due to other teams' collaboration. For instance,
according to }%DIFDELCMD < {\em %%%
\DIFdel{F21}%DIFDELCMD < }%%%
\DIFdel{, delivery delay }%DIFDELCMD < {\em %%%
\DIFdel{``generally happens when
marketing wants to make a splash.''}%DIFDELCMD < } {\em %%%
\DIFdel{F08}%DIFDELCMD < } %%%
\DIFdel{also corroborates  }%DIFDELCMD < {\em %%%
\DIFdel{F21}%DIFDELCMD < } %%%
\DIFdel{by
stating that }%DIFDELCMD < {\em %%%
\DIFdel{``product management }%DIFDELCMD < [%%%
\DIFdel{may}%DIFDELCMD < ] %%%
\DIFdel{change their mind about the
desirability of a feature, or would like to time the release of the feature with
certain external events for marketing reasons.''}%DIFDELCMD < }\\
%DIFDELCMD < 

%DIFDELCMD < \noindent%%%
\textit{\textbf{\DIFdel{Observation~7---The time at which an issue is addressed
during a release cycle and the issue severity are the factors that receive the
highest ratings of importance.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \observation{obs:7} %%%
\DIFdelend \DIFaddbegin \finding{The time at which an issue is addressed
during a release cycle and the issue severity are the factors that receive the
highest ratings of importance.}{find30} \DIFaddend In {\em
...
}\DIFdelend \DIFaddbegin \DIFadd{Regarding }{\em \DIFadd{timing}}\DIFadd{, this }\DIFaddend result is in agreement with our regression models
that are presented in RQ3, in which {\em cycle queue rank} is one of the most
influential variables (see \DIFdelbegin %DIFDELCMD < \hyperref[obs:6]{Observation}%%%
\DIFdel{~\ref{obs:6}}\DIFdelend \DIFaddbegin \hyperref[find22]{Finding}\DIFadd{~\ref{find22}}\DIFaddend ). Indeed,
during the interview, {\em \DIFdelbegin \DIFdel{P06}\DIFdelend \DIFaddbegin \DIFadd{F06}\DIFaddend } further explains that if an issue that is risky
is addressed in the end of a release cycle, such an issue is likely to be
delayed to the next cycle, so that it can receive additional testing. \DIFaddbegin \DIFadd{However,
the importance ranks that were assigned to the }{\em \DIFadd{severity}} \DIFadd{factor contradicts
what we observed in our statistical models (see
}\hyperref[find11]{Finding}\DIFadd{~\ref{find11}). During another interview, participant
}{\em \DIFadd{F23}} \DIFadd{explains that the high importance rating that was given to the }{\em
\DIFadd{severity}} \DIFadd{factor is due to the real severity of an issue---not the severity
value that is often assigned in issue reports. In fact, the study that was
conducted by Herraiz~}\etal \DIFadd{hints that the severity field of an issue report may
be inaccurate~\mbox{%DIFAUXCMD
\cite{Herraiz2008}}%DIFAUXCMD
.
}\DIFaddend 
...
up the shipping process.\DIFdelbegin \footnote{\DIFdel{We did not observe a statistically
...
different team member.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \smartfoot{We did not observe a statistically
	significant difference in delivery delays between issues that are
addressed by the reporters themselves and issues that are addressed by a
different team member.} \DIFaddend As for the {\em \# of comments}, another interviewee
...
\subsection{RQ2: What are \DIFdelbegin \DIFdel{developers' perceptions }\DIFdelend \DIFaddbegin \DIFadd{the perceived impacts }\DIFaddend of \DIFdelbegin \DIFdel{shifting to a
}\DIFdelend rapid
\DIFdelbegin \DIFdel{release cycle}\DIFdelend \DIFaddbegin \DIFadd{releases on delivery delays}\DIFaddend ?}\label{ch5:rq2}
...
to a rapid release cycle \DIFaddbegin \DIFadd{with respect to delivery delays}\DIFaddend . Our findings about
...
\noindent\DIFdelbegin %DIFDELCMD < {%%%
\textit{\textbf{\DIFdel{Theme~6---Management.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \theme{th:6} %%%
\DIFdelend \DIFaddbegin \finding{Management.}{find31} \DIFaddend The shift to a rapid
...
our \DIFdelbegin \DIFdel{observation }\DIFdelend \DIFaddbegin \DIFadd{finding }\DIFaddend that rapid Firefox releases tend to deliver addressed issues
more consistently (see \DIFdelbegin %DIFDELCMD < \hyperref[obs:2]{Observation}%%%
...
\noindent\DIFdelbegin \textit{\textbf{\DIFdel{Theme~7--- Delivery.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \theme{th:7} %%%
\DIFdelend \DIFaddbegin \finding{Delivery.}{find32} \DIFaddend The most recurrent perceived
...
\noindent\DIFdelbegin \textit{\textbf{\DIFdel{Theme~8---Development activities.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \theme{th:8}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \finding{Development.}{find33}
...
\DIFdelbegin \DIFdel{P27}\DIFdelend \DIFaddbegin \DIFadd{E27}\DIFaddend } argues that, by adopting rapid releases, a development team can {\em ``fix
...
\subsection{RQ3: \DIFdelbegin \DIFdel{To what extent do developers }\DIFdelend \DIFaddbegin \DIFadd{Do participants }\DIFaddend agree with our quantitative
...
quantitative findings. More specifically, we aim to understand \DIFdelbegin \DIFdel{to what extent
}\DIFdelend \DIFaddbegin \DIFadd{whether }\DIFaddend our
quantitative findings \DIFdelbegin \DIFdel{agree }\DIFdelend \DIFaddbegin \DIFadd{resonates }\DIFaddend with the participants' perception of \DIFdelbegin \DIFdel{integration
...
\noindent\DIFdelbegin \textit{\textbf{\DIFdel{Theme~9---delivery delay in general.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \theme{th:9}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \finding{delivery delay in general.}{find34}
...
\begin{figure}
	...
	\caption{Proportion of addressed issues that have their \DIFdelbeginFL \DIFdelFL{integration
		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{delivery
		}\DIFaddendFL delayed by a given number of releases. For example, 89\% of the
...
{\em F23} explains \DIFdelbegin \DIFdel{during the interview }\DIFdelend that delivery delay can happen when there are {\em
...
their opinion \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{regarding }\DIFaddend why the data from the Firefox project behaves differently from
...
\noindent\DIFdelbegin \textit{\textbf{\DIFdel{Theme~10---Impact of switching to a rapid release
cycle.}}%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \theme{th:10}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \finding{Impact of switching to a rapid release cycle.}{find35}
...
then ask \DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{whether }\DIFaddend this result resonates with the participants' experience. More
...
We \DIFdelbegin \DIFdel{interviewed }\DIFdelend \DIFaddbegin \DIFadd{could interview only }\DIFaddend two participants that disagree with the results ({\em
...
than rapid release to get fixes in users hands.''} \DIFaddbegin \DIFadd{Unfortunately, we did not
